...
export function getTwitterShareText(trend, currentIndex, title) {
  if (trend === "twitter") {
    return `https://twitter.com/intent/tweet?url=frontend.artrends.ar&text=En%20este%20momento%20${encodeURIComponent(
      title
  ...
  if (trend === "portals.laNacion") {
    return `https://twitter.com/intent/tweet?url=frontend.artrends.ar&text=[${encodeURIComponent(
      title
    )}]%20está%20en%20el%20puesto%20N°%20${
      currentIndex + 1
